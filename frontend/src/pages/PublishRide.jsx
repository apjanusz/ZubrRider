...
function buildAddressQuery(prefix, formData) {
  ...
  if (commaParts.length >= 2) {
    const baseParts = commaParts.filter(
      (part) => part.toLowerCase() !== "poland"
    );
...
function normalizeSuggestion(suggestion) {
  const numberMatchSource = suggestion.label || suggestion.name || "";
  const streetNumberMatch = numberMatchSource.match(/\b(\d+[A-Za-z0-9/-]*)\b/);

  return {
    label: suggestion.label || "",
    city: suggestion.locality || suggestion.city || "",
    street: suggestion.street || "",
    st_number: streetNumberMatch?.[1] || "",
          ...
          {options.map((option) => (
            <button
              ...
              className={`block w-full px-3 py-2 text-left text-sm transition hover:bg-gray-50 ${
                value === option
                  ? "bg-zubr-cream font-semibold text-zubr-dark"
                  : "text-gray-700"
              }`}
            >
              {option}
...
function PublishRide() {
  ...
  useEffect(() => {
    const fetchRoutePreview = async () => {
      ...
      try {
        ...
        setPreviewRoute(null);
        setPreviewError(
          error.response?.data?.detail || "Nie udało się pobrać podglądu trasy."
        );
      } finally {
        ...
        try {
          ...
          setSuggestionError((prev) => ({
            ...
            [prefix]:
              ...
              "Nie udało się pobrać sugestii adresu.",
          }));
        } finally {
          if (latestSuggestionRequest.current[prefix] === requestId) {
            setLoadingSuggestions((prev) => ({ ...prev, [prefix]: false }));
  ...
  }, [formData.start_address, formData.end_address, selectedAddresses]);

  const handleChange = (e) => {
    const { name, value } = e.target;
    const prefix = name.startsWith("start_")
      ? "start"
      : name.startsWith("end_")
      ? "end"
      : null;
    const hadSelectedAddress = prefix
      ? Boolean(selectedAddresses[prefix])
      : false;
    ...
    if (prefix) {
      ...
      setSuggestionError((prev) => ({
        ...prev,
        [prefix]: hadSelectedAddress
          ? "Adres został zmieniony. Wybierz go ponownie z sugestii."
          : "",
      }));
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!selectedAddresses.start || !selectedAddresses.end) {
      setFormError(
        "Wybierz z sugestii potwierdzony adres startowy i docelowy."
      );
      setSubmitting(false);
      return;
    }

    if (!selectedAddresses.start.st_number || !selectedAddresses.end.st_number) {
      setFormError(
        "Nie udało się rozpoznać numeru domu dla jednego z adresów. Wybierz dokładniejszą sugestię."
      );
      setSubmitting(false);
      return;
    }
    ...
    const payload = {
      ...
      start_location: {
        ...
        st_number: selectedAddresses.start.st_number,
      ...
      end_location: {
        ...
        st_number: selectedAddresses.end.st_number,
    ...
    try {
      ...
      const nestedLocationError =
        data?.start_location?.st_number ||
        data?.start_location?.street ||
        data?.end_location?.st_number ||
        data?.end_location?.street;
      const detail =
        nestedLocationError ||
        data?.location ||
        data?.car_id ||
  ...
  if (cars.length === 0) {
    return (
      <div className="text-center mt-10 text-gray-500">
        Przekierowanie do Twoich przejazdów...
      </div>
    );
  }
  ...
  const isCustomDate =
    formData.date &&
    !quickDateOptions.some((option) => option.value === formData.date);
  const isCustomTime =
    formData.time && !quickTimeOptions.includes(formData.time);
  const customTimeParts = splitTimeParts(formData.time);
  const canSubmitRide = Boolean(
    selectedAddresses.start && selectedAddresses.end && formData.car_id
  );
  ...
  return (
    <div className="w-full py-6 sm:py-10">
      <div className="mb-6 flex flex-col gap-4 md:mb-8 md:flex-row md:items-start md:justify-between">
        <div>
          <h1 className="mb-2 text-2xl font-bold text-zubr-dark sm:text-3xl">
            Dodaj nowy przejazd
          </h1>
          <p className="text-sm text-gray-600 sm:text-base">
            Wpisz adresy w formacie ulica i numer, potem miasto, a następnie
            wybierz właściwe sugestie.
          </p>
        </div>
        <button
          type="button"
        ...
        <div className="lg:col-span-2 min-w-0">
          <form onSubmit={handleSubmit} className="space-y-6 sm:space-y-8">
            <div className="rounded-lg border border-gray-100 bg-gray-50 p-4 sm:p-5">
              <h3 className="mb-4 flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-gray-700">
              ...
              <p className="mt-3 text-sm text-gray-500">
                Preferowany format: ulica i numer, potem miasto.
              </p>
              {showStartStatus && (
              ...
              {formData.start_address &&
                ...
                !suggestionError.start && (
                  <p className="mt-3 text-sm text-amber-600">
                    Wybierz adres startowy z listy sugestii.
                  </p>
                )}
                ...
                <div className="mt-3 space-y-2">
                  {locationSuggestions.start.map((suggestion, index) => (
                    <button
                      ...
                      className="w-full rounded-lg border border-gray-200 bg-white p-3 text-left transition hover:border-zubr-gold"
                    >
            ...
            <div className="rounded-lg border border-gray-100 bg-gray-50 p-4 sm:p-5">
              ...
              {formData.end_address &&
                ...
                !suggestionError.end && (
                  <p className="mt-3 text-sm text-amber-600">
                    Wybierz adres docelowy z listy sugestii.
                  </p>
                )}
              ...
              <div className="space-y-3 rounded-xl border border-gray-200 bg-white p-4">
                ...
                {formData.date && (
                  <p className="text-sm text-gray-600">
                    Wybrana data:{" "}
                    <span className="font-semibold text-zubr-dark">
                      {formData.date}
                    </span>
                  </p>
                )}
            ...
            <button
              ...
              className="flex w-full items-center justify-center gap-3 rounded-xl bg-zubr-dark py-4 text-base font-bold text-white shadow-lg transition hover:bg-green-800 disabled:opacity-50 sm:text-lg"
            >
              {submitting ? "Trwa publikowanie..." : <>Opublikuj przejazd</>}
            </button>
          </form>
        </div>

        <div className="lg:col-span-1 min-w-0">
          <div className="space-y-4 rounded-xl border border-gray-100 bg-white p-4 shadow-md sm:p-5 lg:sticky lg:top-24">
            <div>
              <h2 className="mb-3 flex items-center gap-2 text-lg font-semibold text-zubr-dark">
            ...
            <div className="space-y-2 text-sm text-gray-600">
              <div>
                ...
                {selectedAddresses.start?.label ||
                  "Wybierz adres startowy z sugestii"}
              </div>
              <div>
                <span className="font-medium text-gray-800">Koniec:</span>{" "}
                {selectedAddresses.end?.label ||
                  "Wybierz adres docelowy z sugestii"}
              </div>
            </div>
            ...
            {loadingPreview ? (
              ...
              <div className="space-y-1 rounded-lg bg-zubr-cream p-4 text-sm text-gray-700">
                <div>
